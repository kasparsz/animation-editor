<template>
    <div class="playback-bar" v-on:mousedown="drag($event, 'pan', screenPanUnits)" v-on:mousewheel="handleMouseWheel">
        <ul v-bind:style="{ transform: 'translateX(' + screenPan.x + 'px)'}" class="playback-bar__ticks">
            <li v-for="tick in ticks" v-bind:style="{ width: tick.width + 'px'}" class="playback-bar__tick">
...
<script>
    ...
    import SliderBar from './../common/slider-bar.vue';
    ...
    export default {
        ...
        methods: {
            handlePlaybackRangeChange (event) {
                if (event.property === 'min' || event.property === 'line') {
                    this.$store.dispatch('playbackMinimum', event.min);
                }
                if (event.property === 'max' || event.property === 'line') {
                    this.$store.dispatch('playbackMaximum', event.max);
                }
            },
