<template>
    <li class="layer" v-bind:class="{ 'is-active': isActive, 'layer--property': isProperty }">
        ...
        <ul class="layer__children" v-if="data.expanded">
            <layer v-for="layer in data.children" v-bind:data="layer" v-bind:depth="depth+1"></layer>
        </ul>
    </li>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            icon () {
                const layer    = this.$store.getters.layers[this.data.id];
                ...
                if (icon === 'folder' && this.data.expanded) {
                    icon = 'folder-open';
                }
